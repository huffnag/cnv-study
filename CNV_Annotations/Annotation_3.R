# Deletions only for regression tables
...
markers <- read.csv(paste(path,'chipmarkers.csv',sep = '/'))
...
cnv.data[,l.df+3] <- 0 # initialize an empty col for pHI
...
for(chrome in 1:22){ # for each chromosome
  ...
  ploi.loop <- ploi %>% # filter valu es so that only genes which potentially fall completely within a cnv are kept
    ...
    select(start_position,end_position,oe_lof_upper,ngenes,pLI,pHI)
...
lof.data.v3 <- lof.data.v2 %>% 
  filter(Type %in% duplist) %>% # set duplications to zero 
  mutate(cnv.lof=0,
         sizedel=0,
         cnv.ngenes=0,
         cnv.pLI=0,
...
lof.data.v4 <- lof.data.v2 %>% # deletions
...
test <- lof.data.v5 %>% 
  rename(chipids=chip) %>%
  mutate(chipids=paste(chipids,'.int.csv',sep = ''))

lof.data.v5 <- merge(test,markers,by='chipids',all.y = FALSE)
...
subject.annotated <- lof.data.v5 %>% 
  ...
  select(cag_id,LOF,Burden,ngenes,pLI,logLOF,logpLI,pHI,nmarkers)

save(subject.annotated,file = paste(path,'cag.annotated.3march.Rdata',sep = '/'))
